const Advertisement = require("../models/advertisement.model.js");

// Create and Save a new advertisement
exports.create = (req, res) => {
           // Validate request
        if (!req.body) {
          res.status(400).send({
            message: "le contenue ne peut être vide!"
          });
        }
        // Create a advertisement
        const advertisement = new Advertisement({
          contrat: req.body.contrat,
          description: req.body.description,
          nom: req.body.nom,
          salaire: req.body.salaire,
          teampsDeTravail: req.body.teampsDeTravail,
          companiesId: req.body.companiesId,

        });
      
        // Save advertisement in the database
        Advertisement.create(advertisement, (err, data) => {
          if (err)
            res.status(500).send({
              message:
                err.message || "Some error occurred while creating the advertisement."
            });
          else res.send(data);
        }); 
};

// Retrieve all advertisement from the database.
exports.findAll = (req, res) => {
  Advertisement.getAll((err, data) => {
        if (err)
          res.status(500).send({
            message:
              err.message || "Some error occurred while retrieving advertisement."
          });
        else res.send(data);
      });
};

// Find a single advertisement with a advertisementId
exports.findOne = (req, res) => {
  Advertisement.findById(req.params.advertisementId, (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            res.status(404).send({
              message: `Not found Information with id ${req.params.advertisementId}.`
            });
          } else {
            res.status(500).send({
              message: "Error retrieving Information with id " + req.params.advertisementId
            });
          }
        } else res.send(data);
      });
};

exports.findOneByComp = (req, res) => {
  Advertisement.findByIdComp(req.params.companiesId, (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            res.status(404).send({
              message: `Not found ad with id ${req.params.companiesId}.`
            });
          } else {
            res.status(500).send({
              message: "Error retrieving ad with id " + req.params.companiesId
            });
          }
        } else res.send(data);
      });
};

// Update a advertisement identified by the advertisementId in the request
exports.update = (req, res) => {
     // Validate Request
  if (!req.body) {
    res.status(400).send({
      message: "Content can not be empty!"
    });
  }
  Advertisement.updateById(
    req.params.advertisementId,
    new Advertisement(req.body),
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
            message: `Not found advertisement with id ${req.params.advertisementId}.`
          });
        } else {
          res.status(500).send({
            message: "Error updating advertisement with id " + req.params.advertisementId
          });
        }
      } else res.send(data);
    }
  );  
};

// Delete a advertisement with the specified customerId in the request
exports.delete = (req, res) => {
  Advertisement.remove(req.params.advertisementId, (err, data) => {
        if (err) {
          if (err.kind === "not_found") {
            res.status(404).send({
              message: `Not found advertisement with id ${req.params.advertisementId}.`
            });
          } else {
            res.status(500).send({
              message: "Could not delete advertisement with id " + req.params.advertisementId
            });
          }
        } else res.send({ message: `advertisement was deleted successfully!` });
      });
};

// Delete all Customers from the database.
exports.deleteAll = (req, res) => {
  Advertisement.removeAll((err, data) => {
        if (err)
          res.status(500).send({
            message:
              err.message || "Some error occurred while removing all advertisement."
          });
        else res.send({ message: `All advertisement were deleted successfully!` });
      });
};
exports.create = (req, res) => {
  // Validate request
  if (!req.body) {
    res.status(400).send({
      message: "Content can not be empty!"
    });
  }
};
